...
describe('Users', function () {
  ...
  describe('/POST user', function () {
    it('it should not POST a user without firstname, lastname, username, password,\n     email, gender, street, city, state, dob, phone', function (done) {
      ...
      request.post('/auth/v1/signup').field('id', '1').field('title', 'mr').field('firstname', '').field('lastname', '').field('username', '').field('password', '').field('email', '').field('gender', '').field('street', '').field('city', '').field('state', '').field('dob', '').field('phone', '').attach('userImage', './travis config.png').end(function (err, res) {
  ...
  */
  ...
  describe('connect.static()', function () {
    it('should serve static files', function (done) {
      ...
      request.get('/usersUploads/travis config.png').end(function (err, res) {
        //delete test image file
        if (_path2.default.resolve('./usersUploads/travis config.png')) {
          _fileSystem2.default.unlink('./usersUploads/travis config.png', function (err) {
            if (err) new Error('oohs something went wrong');
          });
        }
        done();
