...
const _user = require('../server/models/user');
...
describe('Users', () => {
  ...
  describe('/POST user', () => {
    it('it should not POST a user without firstname, lastname, username, password,\n     email, gender, street, city, state, dob, phone', (done) => {
      request.post('/auth/v1/signup').field('id', '1').field('title', 'mr').field('firstname', '')
        .field('lastname', '')
        .field('username', '')
        .field('password', '')
        .field('email', '')
    ...
    it('it should post a user', (done) => {
      request.post('/auth/v1/signup').field('id', '1').field('title', 'mr').field('firstname', 'Justin')
        ...
        .end((err, res) => {
          ...
          if (_user2.default[0].userImage) {
            _fileSystem2.default.unlink(`./${_user2.default[0].userImage}`, (err) => {
              if (err) throw err;
            });
          }
    ...
    it('it should POST username && password and get the particular user ', (done) => {
      ...
      request.post('/auth/v1/login').send({ username: 'justman', password: 'abc' }).end((err, res) => {
        res.should.have.status(200);
        res.body.should.be.a('object');
    ...
    it('it should not get a particular user if POST a wrong username && password', (done) => {
      ...
      _user2.default.push(user);
      request.post('/auth/v1/login').send({ username: 'justin', password: 'abc' }).end((err, res) => {
  ...
  */
  describe('/GET/ user', () => {
    ...
    it('it should GET a user by the given id', (done) => {
      ...
      // Passing user to user model
      _user2.default.push(user);
      request.get(`/api/v1/users/${user.id}`).end((err, res) => {
        res.should.have.status(200);
        res.body.should.be.a('object');
        res.body.Users.should.have.property('firstname').eql('justin');
        res.body.Users.should.have.property('lastname').eql('Ikwuoma');
    ...
    it('it should not GET a user by the given wrong id', (done) => {
      ...
      // Passing user to user model
      _user2.default.push(user);
      request.get('/api/v1/users/14').end((err, res) => {
        res.should.have.status(404);
        res.body.should.be.a('object');
        res.body.message.should.be.eql('User not found');
        res.body.error.should.be.eql(true);
  ...
  describe('/PUT/:id user', () => {
    it('it should UPDATE a user given the id', (done) => {
      ...
      request.put(`/api/v1/users/${user.id}`).field('id', '1').field('title', 'mr').field('firstname', 'Justin')
        .field('lastname', 'Ikwuoma')
    ...
    it('it should not UPDATE a user given the wrong id', (done) => {
      ...
      request.put('/api/v1/users/14').field('id', '1').field('title', 'mr').field('firstname', 'Justin')
        .field('lastname', 'Ikwuoma')
        .field('username', 'sojust')
        .field('password', 'abcd')
        .field('email', 'justin@gmail.com')
    ...
    it('it should UPDATE a user given the id and\n      maintain already existing fields and file if none is entered', (done) => {
      ...
      request.put(`/api/v1/users/${user.id}`).field('id', '1').field('title', 'mr').field('firstname', '')
        .field('lastname', '')
        .field('username', '')
        .field('password', '')
        .field('email', '')
    ...
    it('it should UPDATE a user given the id and replace already existing file', (done) => {
      ...
      const destDir = _path2.default.join('./', 'usersUploads');
  ...
  */
  describe('/DELETE/:id user', () => {
    it('it should DELETE a user given the id', (done) => {
      ...
      request.delete(`/api/v1/users/${user.id}`).end((err, res) => {
    ...
    it('it should not DELETE a user given the wrong id', (done) => {
      ...
      // Passing user to user model
      _user2.default.push(user);
      _chai2.default.request(_app2.default).delete('/api/v1/users/15').end((err, res) => {
        res.should.have.status(404);
        res.body.should.be.a('object');
        res.body.should.have.property('message').eql('User not found');
        res.body.error.should.be.eql(true);
  ...
  describe('connect.static()', () => {
    it('should serve static files', (done) => {
      ...
      request.get('/usersUploads/testFile.png').end(() => {
        // delete test image file
        if (_path2.default.resolve('./usersUploads/testFile.png')) {
          _fileSystem2.default.unlink('./usersUploads/testFile.png', (err) => {
            if (err) throw err;
          });
        }
        done();
