...
describe('Businesses', function () {
  ...
  describe('/POST business', function () {
    ...
    it('it should not CREATE a business without description, businessName, userId,\n    description, location and category', function (done) {
      ...
      request.post('/v1/businesses/').field('businessId', '1').field('businessName', '').field('userId', '').field('description', '').field('location', '').field('category', '').attach('companyImage', './travis config.png').end(function (err, res) {
    ...
    it('it should CREATE a business', function (done) {
      ...
      request.post('/v1/businesses/').field('businessId', '1').field('businessName', 'Sommyj').field('userId', '22').field('description', 'We produce quality products').field('location', 'lagos').field('category', 'Production').attach('companyImage', './travis config.png').end(function (err, res) {
  ...
  */
  ...
  describe('/PUT/:id business', function () {
    ...
    it('it should UPDATE a business given the id', function (done) {
      ...
      request.put('/v1/businesses/' + business[0].businessId).field('businessId', '1').field('businessName', 'Sommyj Enterprise').field('userId', '22').field('description', 'We sale quality products').field('location', 'port-harcourt').field('category', 'Sales').attach('companyImage', './travis config.png').end(function (err, res) {
    ...
    it('it should not UPDATE a business given the wrong id', function (done) {
      ...
      request.put('/v1/businesses/12').field('businessId', '1').field('businessName', 'Sommyj Enterprise').field('userId', '22').field('description', 'We sale quality products').field('location', 'port-harcourt').field('category', 'Sales').attach('companyImage', './travis config.png').end(function (err, res) {
        res.should.have.status(404);
        res.body.should.be.a('object');
        res.body.should.have.property('error').eql(true);
    ...
    it('it should UPDATE a business given the id and replace already existing file', function (done) {
      ...
      var filename = 'travis config.png';
      ...
      _fileSystem2.default.access(destDir, function (err) {
        if (err) _fileSystem2.default.mkdirSync(destDir);

        copyFile(src, _path2.default.join(destDir, filename));
      });

      request.put('/v1/businesses/11').field('businessId', '13').field('businessName', 'Sommyj Enterprise').field('userId', '22').field('description', 'We sale quality products').field('location', 'port-harcourt').field('category', 'Sales').attach('companyImage', './travis config.png').end(function (err, res) {
        res.should.have.status(200); // 'success' status
        res.body.should.be.a('object');
        res.body.should.have.property('Businesses');
        res.body.Businesses.should.have.property('businessId').eql('11');
  ...
  */
  ...
  describe('connect.static()', function () {
    it('should serve static files', function (done) {
      ...
      request.get('/businessesUploads/travis config.png').end(function (err, res) {
        //delete test image file
        if (_path2.default.resolve('./businessesUploads/travis config.png')) {
          _fileSystem2.default.unlink('./businessesUploads/travis config.png', function (err) {
            if (err) new Error('oohs something went wrong');
          });
        }
        done();
