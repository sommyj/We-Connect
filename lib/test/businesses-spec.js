...
describe('Businesses', function () {
  ...
  describe('/POST business', function () {
    ...
    it('it should POST a business', function (done) {
      request.post('/v1/businesses/').field('businessId', '1').field('businessName', 'Sommyj').field('userId', '22').field('description', 'We produce quality products').field('location', 'lagos').field('category', 'Production').attach('companyImage', './testFile.png').end(function (err, res) {
        ...
        if (_business2.default[0].companyImage) {
          deleteFile('./' + _business2.default[0].companyImage);
        }
        done();
      });
    });

    it('it should POST a business without image', function (done) {
    ...
    it('it should not POST a business when image file type not jpg/png', function (done) {
  ...
  */
  ...
  describe('/PUT/:id business', function () {
    ...
    it('it should not UPDATE a business when image file type not jpg/png', function (done) {
      ...
      request.put('/v1/businesses/11').field('businessId', '1').field('businessName', 'Sommyj').field('userId', '22').field('description', 'We produce quality products').field('location', 'lagos').field('category', 'Production').attach('companyImage', './testFileType.txt').end(function (err, res) {
        res.should.have.status(403);
        res.body.should.be.a('object');
        res.body.should.have.property('message').eql('Only .png and .jpg files are allowed!');
        res.body.should.have.property('error').eql(true);
    ...
    it('it should not UPDATE a business\n    when image file size is larger than 2mb', function (done) {
      ...
      request.put('/v1/businesses/11').field('businessId', '1').field('businessName', 'Sommyj').field('userId', '22').field('description', 'We produce quality products').field('location', 'lagos').field('category', 'Production').attach('companyImage', './testFileSize.jpg').end(function (err, res) {
        res.should.have.status(403);
        res.body.should.be.a('object');
        res.body.should.have.property('message').eql('file should not be more than 2mb!');
        res.body.should.have.property('error').eql(true);
