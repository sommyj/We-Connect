        ...
        if (Business.businessId === req.params.businessId) {
          ...
          if (req.file) {
            if (Business.companyImage) {
              _fileSystem2.default.unlink(`./${Business.companyImage}`, (err) => {
                if (err) return handleError(err, res);
              });
            }
          }
